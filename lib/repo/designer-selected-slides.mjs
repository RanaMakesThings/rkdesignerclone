...
const normalizeStampedSlideRoot = (stampedDirPath, logicalSlideDirPath) => {
  const raw = String(stampedDirPath ?? "").trim();
  if (!raw) {
    return logicalSlideDirPath;
  }
  const versionMarker = "/versions/";
  const versionIndex = raw.indexOf(versionMarker);
  if (versionIndex >= 0) {
    return raw.slice(0, versionIndex);
  }
  return raw;
};
...
const buildSlideRoot = async ({
  ...
  const resolvedRootDirPath = normalizeStampedSlideRoot(
    stampedDirPath,
    logicalSlideDirPath
  );
  const resolvedRootAbsolutePath = resolve(repoRoot, resolvedRootDirPath);
  const usingLogicalFallbackRoot = !stampedDirPath || resolvedRootDirPath !== stampedDirPath;
  const resolvedAssets = resolveSlideCanonicalAssets({
    repoRoot,
    slideAssetsManifestPath: slide.paths?.assetsManifest?.trim() || null,
    deckAssetsManifestPath,

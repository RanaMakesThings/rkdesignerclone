...
const buildLineageStatuses = ({
  ...
  const stampedDirLooksLikeVersionBundle = /^version-\d+/i.test(stampedDirBase ?? "");
  if (
    stampedDirBase &&
    !identityAliases.has(stampedDirBase) &&
    !(stampedDirLooksLikeVersionBundle && canonical.selectedVariantIdResolved)
  ) {
...
export const buildProjectManifest = async ({
  ...
  const counts = {
    ...
    withSelectedPreview: orderedSlides.filter(
      (slide) =>
  ...
  return {
    ...
    studioTheme:
      typeof deckSpec?.studioTheme === "string" && deckSpec.studioTheme.trim()
        ? deckSpec.studioTheme.trim()
        : null,

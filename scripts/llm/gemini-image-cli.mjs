...
const main = async () => {
  await yargs(hideBin(process.argv))
    .scriptName("gemini:image")
    .usage("Usage: npm run gemini:image -- [options]")
    .option("prompt", {
      type: "string",
    ...
    .option("aspect-ratio", {
      ...
      describe: "Requested image aspect ratio, e.g. 16:9 or 1:1",
    })
    .option("image-size", {
      type: "string",
      default: DEFAULT_GEMINI_IMAGE_SIZE,
      describe: "Requested image size when supported, e.g. 1K or 2K",
    })
    .option("slug", {
      type: "string",
    ...
    .then(async (argv) => {
      ...
      const result = await generateGeminiImages({
        ...
        aspectRatio: String(argv.aspectRatio),
        imageSize: String(argv.imageSize),
        images: Array.isArray(argv.image) ? argv.image.map((value) => resolve(String(value))) : [],
      });
      const referenceImages = Array.isArray(argv.image)
      ...
      process.stdout.write(`Generated Gemini image artifacts\n${artifacts.dir}\n`);

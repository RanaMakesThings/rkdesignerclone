...
const main = async () => {
  await yargs(hideBin(process.argv))
    .scriptName("openai:html")
    .usage("Usage: npm run openai:html -- [options]")
    .option("prompt", {
      type: "string",
    ...
    .option("model", {
      type: "string",
      default: DEFAULT_OPENAI_HTML_MODEL,
    ...
    .option("max-output-tokens", {
      type: "number",
      default: DEFAULT_OPENAI_HTML_MAX_OUTPUT_TOKENS,
    ...
    .option("reasoning", {
      type: "string",
      describe: "Reasoning effort or a JSON reasoning object",
    })
    .option("json", {
      type: "boolean",
      default: false,
    ...
    .then(async (argv) => {
      ...
      const result = await generateOpenAIHtml({
        ...
        reasoning: argv.reasoning ? String(argv.reasoning) : undefined,
      ...
      process.stdout.write(`Generated OpenAI HTML artifacts\n${artifacts.dir}\n`);
      process.stdout.write(`${artifacts.htmlPath}\n`);
      process.stdout.write(`${artifacts.basename}\n`);
    })
    .catch((error) => {

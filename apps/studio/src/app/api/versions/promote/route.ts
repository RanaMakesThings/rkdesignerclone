...
const promoteSlideVersionViaCli = async ({
  projectId,
  slideId,
  versionId,
}: {
  ...
  if (!payload?.ok || !payload.slideId || !payload.versionId || !payload.versionDir) {
    throw new Error("Version promotion returned an invalid response.");
  }
  return payload;
};
...
const getErrorMessage = (error: unknown) => {
  ...
  return "Version promotion failed.";
};
...
export async function POST(request: Request) {
  ...
  try {
    const promoted = await promoteSlideVersionViaCli({
      projectId,
      slideId,
      versionId,
    });
